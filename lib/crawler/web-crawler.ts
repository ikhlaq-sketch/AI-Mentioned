...
export async function crawlWebsite(website: any) {
  ...
  try {
    ...
    await service.from('crawl_data').insert({
      ...
      crawl_method: 'standard',
    });

    await service
      .from('websites')
      .update({ status: 'active', last_crawled_at: new Date().toISOString() })
      .eq('id', website.id);
...
async function deepSearchFallback(
  website: any,
  service: any,
  domain: string,
  existingData?: any
  ...
  await service.from('crawl_data').insert({
    website_id: website.id,
    user_id: website.user_id,
    page_title: existingData?.pageTitle || '',
    meta_description: existingData?.metaDesc || '',
    h1_headings: existingData?.h1s || [],
    h2_headings: existingData?.h2s || [],
    schema_markup: existingData?.schemas || [],
    crawl_method: 'deep_search',
    products_services: [aiResponse],
  });
