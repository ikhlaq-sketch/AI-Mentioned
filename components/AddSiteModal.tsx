  ...
  return (
    <>
      <button
        ...
        className="w-full sm:w-auto bg-emerald-600 hover:bg-emerald-700 text-white px-4 sm:px-5 py-2.5 rounded-xl text-sm font-medium transition-all shadow-lg shadow-emerald-200 flex items-center justify-center gap-2"
      >
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl border border-gray-200 shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto relative">
            {/* Close Button */}
            <button
              onClick={() => { if (!loading && auditProgress === 0) setOpen(false); }}
            ...
            {auditProgress > 0 ? (
              ...
              <>
                ...
                <div className="flex items-center justify-center gap-1 sm:gap-2 pt-6 pb-4 px-4 sm:px-6 bg-gray-50 border-b border-gray-100 overflow-x-auto">
                  {steps.map((s, i) => (
                    <div key={s.num} className="flex items-center gap-1 sm:gap-2 whitespace-nowrap">
                      <div className={`w-7 h-7 sm:w-8 sm:h-8 rounded-full flex items-center justify-center text-xs sm:text-sm font-semibold transition-all flex-shrink-0 ${step >= s.num ? 'bg-emerald-600 text-white' : 'bg-gray-200 text-gray-400'}`}>
                        {step > s.num ? '✓' : s.num}
                      </div>
                      <span className={`text-xs sm:text-sm font-medium hidden xs:inline ${step >= s.num ? 'text-emerald-600' : 'text-gray-400'}`}>
                        {s.label}
                      </span>
                      {i < 2 && <div className={`w-4 sm:w-8 h-0.5 ${step > s.num ? 'bg-emerald-400' : 'bg-gray-200'}`} />}
                    </div>
                  ))}
                </div>
                  ...
                  {step === 1 && (
                    <div className="space-y-4">
                      <div>
                        ...
                        <div className="relative mt-1">
                          ...
                          <input
                            ...
                            className="w-full bg-gray-50 border border-gray-200 rounded-xl pl-10 pr-4 py-3 text-gray-900 focus:outline-none focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 transition-all"
                          />
                        </div>
                      </div>
                      <div>
                        ...
                        <input
                          ...
                          className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-gray-900 mt-1 focus:outline-none focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 transition-all"
                        />
                      </div>
                      <div>
                      ...
                      <button
                        ...
                        onClick={() => setStep(2)}
                        className="w-full bg-emerald-600 hover:bg-emerald-700 text-white py-3 rounded-xl font-medium transition-all disabled:opacity-50 flex items-center justify-center gap-2"
                      >
                  ...
                  {step === 2 && (
                    <div className="space-y-4">
                      <p className="text-sm text-gray-500">
                        Add competitors to track their AI visibility{' '}
                        <span className="text-gray-400">(optional)</span>
                      </p>
                      {competitors.map((comp, idx) => (
                        <div key={idx} className="flex flex-col sm:flex-row gap-2 items-start bg-gray-50 p-3 rounded-xl border border-gray-200">
                          <div className="flex-1 w-full space-y-2">
                            <input
                              ...
                              className="w-full bg-white border border-gray-200 rounded-lg px-3 py-2 text-gray-900 text-sm focus:outline-none focus:border-emerald-400"
                            />
                            <input
                              type="text"
                              value={comp.brand_name}
                              onChange={(e) => updateCompetitor(idx, 'brand_name', e.target.value)}
                          ...
                          <button
                            ...
                            className="text-gray-400 hover:text-red-500 mt-1 sm:mt-0 flex-shrink-0"
                          >
                            <Trash2 size={16} />
                          </button>
                        </div>
                      ...
                      <div className="flex flex-col sm:flex-row gap-3 pt-2">
                        <button
                          onClick={() => setStep(1)}
                          className="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-700 py-3 rounded-xl font-medium flex items-center justify-center gap-2"
                        >
                          <ChevronLeft size={16} /> Back
                        </button>
                        <button
                          onClick={() => setStep(3)}
                          className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white py-3 rounded-xl font-medium flex items-center justify-center gap-2"
                        >
                          Continue <ChevronRight size={16} />
                        </button>
                      </div>
                  ...
                  {step === 3 && (
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 xs:grid-cols-2 gap-3">
                        <button
                          ...
                          className={`p-4 rounded-xl border-2 text-left transition-all ${
                            scanMode === 'auto' ? 'border-emerald-400 bg-emerald-50' : 'border-gray-200 hover:border-gray-300'
                          }`}
                        >
                        ...
                        <button
                          ...
                          className={`p-4 rounded-xl border-2 text-left transition-all ${
                            scanMode === 'manual' ? 'border-emerald-400 bg-emerald-50' : 'border-gray-200 hover:border-gray-300'
                          }`}
                        >
                      ...
                      <button
                        ...
                        disabled={loading}
                        className="w-full bg-emerald-600 hover:bg-emerald-700 text-white py-3 rounded-xl font-medium disabled:opacity-50 flex justify-center items-center gap-2 transition-all"
                      >
