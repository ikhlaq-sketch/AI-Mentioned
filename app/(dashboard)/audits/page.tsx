  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        {/* Optional: Add filter/export button here */}
      </div>
      ...
      {audits && audits.length > 0 ? (
            ...
            <table className="w-full text-sm min-w-[700px]">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200">
                  ...
                  <th className="text-left p-3 sm:p-4 text-gray-500 font-semibold text-xs sm:text-sm">Status</th>
                ...
                {audits.map((audit: any) => (
                  <tr key={audit.id} className="border-t border-gray-100 hover:bg-emerald-50/30 transition-colors">
                    <td className="p-3 sm:p-4">
                      <p className="text-gray-900 font-semibold text-xs sm:text-sm truncate max-w-[120px] sm:max-w-none">
                        {audit.websites?.brand_name || audit.websites?.domain}
                      </p>
                      <p className="text-[10px] sm:text-xs text-gray-400 truncate">{audit.websites?.domain}</p>
                    </td>
                    <td className="p-3 sm:p-4">
                      <span className={`px-2 py-1 text-[10px] sm:text-xs rounded-full font-semibold capitalize ${
                    ...
                    <td className="p-3 sm:p-4">
                      <span className={`inline-flex items-center gap-1.5 px-2 py-1 text-[10px] sm:text-xs rounded-full font-semibold ${
        ...
        <div className="text-center py-16 sm:py-20 bg-white border border-gray-200 rounded-2xl">
          ...
          <p className="text-gray-500 text-sm max-w-md mx-auto px-4">
            Your audit history will appear here after you add a website and run your first scan.
          </p>
