...
export default async function DashboardPage() {
  ...
  // ANTI-GLITCH FEATURE: Detect Plan Limit Violations (e.g., Downgrades)
  ...
  return (
    <div className="p-6 relative">
      ...
      {isOverLimit && (
        <div className="absolute inset-0 z-50 bg-gray-900/40 backdrop-blur-md flex items-center justify-center p-4 min-h-[80vh] rounded-2xl">
          <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full p-6 border border-gray-100 text-center animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 text-left mb-6">
              ...
              <div className="space-y-2 max-h-48 overflow-y-auto">
                {sites?.map((site: any) => (
                  <div key={site.id} className="flex items-center justify-between bg-white border border-gray-200 rounded-lg p-3 text-sm">
                    <div>
                      <p className="font-semibold text-gray-900">{site.domain}</p>
                      <p className="text-xs text-gray-500">{site.brand_name}</p>
                    </div>
                    {/* Direct link redirection to manage and remove properties */}
                    <Link href="/sites" className="inline-flex items-center gap-1 px-3 py-1.5 bg-red-50 hover:bg-red-100 text-red-600 rounded-lg font-medium text-xs transition-colors">
