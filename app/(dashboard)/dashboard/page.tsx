...
import { BarChart3, Globe, Search, TrendingUp, Sparkles, ArrowRight, Zap } from 'lucide-react';
    ...
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
        <StatCard
          ...
          label="Websites"
          value={siteCount ?? 0}
          trend={siteCount && siteCount > 0 ? 'active' : undefined}
        />
        <StatCard
          icon={<Search className="text-emerald-400" />}
          label="Audits"
          value={auditCount ?? 0}
          trend={auditCount && auditCount > 0 ? 'active' : undefined}
        />
        <StatCard
        ...
        <StatCard
          ...
          trend={profile && profile.queries_used / profile.queries_limit > 0.7 ? 'warning' : undefined}
        />
      </div>
      ...
      <section className="mb-10">
        <div className="flex items-center justify-between mb-4">
          ...
          {sites && sites.length > 0 && (
            <Link href="/sites" className="text-sm text-emerald-400 hover:text-emerald-300 flex items-center gap-1">
        ...
        {sites && sites.length > 0 ? (
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {sites.map((site: any) => (
              <Link
                key={site.id}
                href={`/sites/${site.id}`}
                className="group bg-white border border-gray-200 rounded-2xl p-5 hover:border-emerald-300 hover:shadow-lg hover:shadow-emerald-50 transition-all duration-300 hover:-translate-y-1"
              >
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      ...
                      <span className={`w-1.5 h-1.5 rounded-full ${
                        site.status === 'active' ? 'bg-emerald-500' : 'bg-gray-300'
                      }`} />
                    </div>
                  </div>
                  <div className="text-right">
                    <span className={`text-2xl font-bold ${
                      ...
                      site.visibility_score >= 40 ? 'text-amber-500' :
                      'text-red-500'
                    }`}>
          ...
          <div className="text-center py-16 bg-white border border-gray-200 rounded-2xl">
            ...
            <p className="text-gray-900 font-semibold text-lg mb-2">No websites yet</p>
            ...
            <Link
              href="/sites"
              className="inline-flex items-center gap-2 px-6 py-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-full font-medium transition-all shadow-lg shadow-emerald-200"
            >
              <Zap size={16} />
      ...
      <section className="mb-10">
        <div className="flex items-center justify-between mb-4">
          ...
          {recentAudits && recentAudits.length > 0 && (
            <Link href="/audits" className="text-sm text-emerald-400 hover:text-emerald-300 flex items-center gap-1">
              View All <ArrowRight size={14} />
            </Link>
          )}
        </div>
        {recentAudits && recentAudits.length > 0 ? (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50">
                  <th className="text-left p-4 text-gray-500 font-medium">Website</th>
                  ...
                  <tr key={audit.id} className="border-t border-gray-100 hover:bg-emerald-50/30 transition-colors">
                    ...
                    <td className="p-4">
                      <span className={`font-semibold ${
                        ...
                        audit.visibility_score >= 40 ? 'text-amber-500' :
                        'text-red-500'
                      }`}>
                    ...
                    <td className="p-4 text-gray-400">{new Date(audit.created_at).toLocaleDateString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="text-center py-12 bg-white border border-gray-200 rounded-2xl">
            ...
            <p className="text-gray-500 text-sm">No audits yet. Your first audit runs when you add a website.</p>
          </div>
        )}
      </section>
      ...
      <section>
        <div className="flex items-center justify-between mb-4">
          ...
          {topRecs && topRecs.length > 0 && (
            <Link href="/recommendations" className="text-sm text-emerald-400 hover:text-emerald-300 flex items-center gap-1">
              View All <ArrowRight size={14} />
            </Link>
          )}
        </div>
        {topRecs && topRecs.length > 0 ? (
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {topRecs.map((rec: any) => (
              <div
                key={rec.id}
                className="bg-white border border-gray-200 rounded-2xl p-5 hover:border-emerald-300 hover:shadow-md transition-all duration-300"
              >
          ...
          <div className="text-center py-12 bg-white border border-gray-200 rounded-2xl">
            ...
            <p className="text-gray-500 text-sm">No recommendations yet. Complete an audit to get AI-powered fixes.</p>
          </div>
        )}
      </section>
    </div>
