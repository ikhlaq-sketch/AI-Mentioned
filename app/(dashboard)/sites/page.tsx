    ...
    <div className="p-6">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">{sites?.length || 0} / {profile?.sites_limit || 0} sites used</p>
        </div>
      ...
      {!sites || sites.length === 0 ? (
        <div className="text-center py-20 bg-white border border-gray-200 rounded-2xl">
          ...
          <p className="text-gray-500 text-sm mb-6 max-w-md mx-auto">Add your first website to start monitoring AI visibility.</p>
          {canAddSite && <AddSiteModal />}
        </div>
      ) : (
          ...
          {sites.map((site: any) => (
            <Link key={site.id} href={`/sites/${site.id}`}
              ...
              <div className="flex justify-between items-start mb-3">
                <div>
                  <p className="font-bold text-gray-900 group-hover:text-emerald-600 transition-colors">{site.domain}</p>
                  <p className="text-sm text-gray-500">{site.brand_name}</p>
                </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-emerald-600 group-hover:translate-x-1 transition-transform text-sm font-medium flex items-center gap-1">View <ArrowRight size={14} /></span>
              </div>
            </Link>
          ))}
        </div>
